...
sample_prefixes       <- gsub(".f(ast)?q.gz", "", (list.files(read_file_folder, pattern = "*.f(ast)?q.gz")))
...
for(prefix in sample_prefixes) {
  ...
  if(!file.exists(paste(outputPrefix,"_R1.fastq.gz", sep=""))) {
    ...
    read1 = file.path(read_file_folder, paste(prefix, ".fastq.gz", sep=""))
    if(!file.exists(read1)) { read1 = file.path(read_file_folder, paste(prefix, ".fq.gz", sep="")) }
...
qc_trimmed$sample = gsub("_R1$", "" , qc_trimmed$sample)
